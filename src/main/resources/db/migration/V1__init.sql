...
CREATE TABLE IF NOT EXISTS knowledge_subscription (
    ...
    last_ingested_at TIMESTAMP NULL COMMENT '上次拉取时间（用于增量拉取/避免重复拉取）',
...
CREATE TABLE IF NOT EXISTS knowledge_article (
    ...
    attractiveness_score DECIMAL(10,4) NULL COMMENT '吸引力评分（越高越值得优先阅读）',
    view_count INT NULL COMMENT '浏览量（若可解析）',
    comment_count INT NULL COMMENT '评论量（若可解析）',
    like_count INT NULL COMMENT '点赞/喜欢数（若可解析）',
    share_count INT NULL COMMENT '转发/分享数（若可解析）',
    attractiveness_signals_json LONGTEXT NULL COMMENT '吸引力信号明细（JSON，便于排查）',
    scored_at TIMESTAMP NULL COMMENT '吸引力评分计算时间',
    ...
    KEY idx_knowledge_article_score (attractiveness_score),
